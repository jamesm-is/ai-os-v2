...
1. Read `projects/<project-name>/status.md`. Confirm `stage: handed-off-to-github`.
   - If `stage: slices-validated`, tell the user to run the handoff step first (see `.agents/skills/handoff/SKILL.md`).
...
| **Python** | `python:3.12-bookworm` | `pip install -e ".[dev]" && pip install build` | `pytest` | `pyright` or `mypy` (from PRD) | `python -m build` (if `pyproject.toml` has a build-backend) or `true` (no-op) |
