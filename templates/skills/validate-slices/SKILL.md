...
description: Audit generated issues to verify they are true vertical slices, not horizontal layer-based tasks. Use after /to-issues, when reviewing issue quality, or when the user says "validate slices", "check these issues", or "audit issues".
---
...
1. Read the relevant PRD from `docs/` (either `docs/prd.md` or `docs/prd-<feature-slug>.md`).
2. Read `CONTEXT.md` for glossary vocabulary.
3. Read all issue files in `docs/issues/` that belong to the feature being validated.
...
- **50–79% pass** — revise failed and borderline issues before continuing.
...
1. Save the audit report to `docs/slice-audit.md` (or `docs/slice-audit-<feature-slug>.md` for feature-specific audits).
2. If issues were rewritten, update the corresponding files in `docs/issues/`.
3. Tell the user the next step: proceed to `/to-sandcastle` (if passing), fix issues (if borderline), or re-run `/to-issues` (if failing).
