...
- Skills live in `.claude/skills/` (Claude Code) and `.agents/skills/` (Codex). Eleven skills total: pipeline (align, to-prd, to-issues, validate-slices, handoff), post-handoff (to-sandcastle), architecture (improve-codebase-architecture), utility (relay, relay-handoff, preflight).
...
Once a project is handed off, it is **fully self-governing**. Handoff installs skills in both `.claude/skills/` (Claude Code) and `.agents/skills/` (Codex) — pipeline (align, to-prd, to-issues, validate-slices, to-sandcastle), architecture (improve-codebase-architecture), and utility (relay, relay-handoff). It also generates a kanban board (`docs/kanban.html` + `docs/kanban-state.json`) for visual issue tracking and an `AGENTS.md` boot file for Codex. This means:
