...
const buildSandboxHandle = (ctx, close) => {
    const { branch, worktreePath, hostRepoDir, sandboxRepoDir, sandboxLayer, providerHandle, applyToHost, timeouts, } = ctx;
    const sandboxHandle = {
        branch,
        ...
        run: async (runOptions) => {
            ...
            const runLayer = Layer.mergeAll(reuseFactoryLayer, runDisplayLayer, agentStreamEmitterLayer);
            let result;
            try {
...
export const createSandbox = async (options) => {
    ...
    const { hostRepoDir, worktreePath, providerHandle, sandboxLayer, sandboxRepoDir, } = await Effect.runPromise(Effect.gen(function* () {
        const hostRepoDir = yield* resolveCwd(options.cwd);
        yield* WorktreeManager.pruneStale(hostRepoDir).pipe(Effect.catchAll(() => Effect.void));
        const { path: worktreePath } = yield* WorktreeManager.create(hostRepoDir, { branch, baseBranch: options.baseBranch });
        const prepared = yield* Effect.gen(function* () {
            // Copy files (bind-mount/no-sandbox only; isolated copies in startSandbox).
            if (options.copyToWorktree &&
                options.copyToWorktree.length > 0 &&
                options.sandbox.tag !== "isolated") {
                yield* copyToWorktree(options.copyToWorktree, hostRepoDir, worktreePath, options.timeouts?.copyToWorktreeMs);
            }
            ...
            if (isTestMode) {
                ...
                const startResult = yield* provider.tag === "isolated"
                    ...
                    : provider.tag === "none"
                        ...
                        : resolveGitMounts(join(hostRepoDir, ".git")).pipe(Effect.provide(NodeFileSystem.layer), Effect.catchAll(() => Effect.succeed([])), 
                        // Patch git mounts for Windows worktree compatibility (ADR-0006)
                        Effect.flatMap((gitMounts) => Effect.tryPromise({
                            try: () => patchGitMountsForWindows(gitMounts, worktreePath, SANDBOX_REPO_DIR),
                            catch: (e) => new Error(`Failed to patch git mounts: ${e instanceof Error ? e.message : String(e)}`),
            ...
            if (sandboxOnReady?.length || hostOnReady?.length) {
                ...
                }).pipe(Effect.provide(sandboxLayer), Effect.onError(() => providerHandle
                    ? Effect.promise(() => providerHandle.close().catch(() => { }))
                    : Effect.void));
            }
            return { providerHandle, sandboxLayer, sandboxRepoDir };
        }).pipe(Effect.onError(() => WorktreeManager.remove(worktreePath).pipe(Effect.catchAll(() => Effect.void))));
    ...
    const doClose = async () => {
        ...
        return Effect.runPromise(Effect.gen(function* () {
            ...
            const isDirty = yield* WorktreeManager.hasUncommittedChanges(worktreePath).pipe(Effect.catchAll(() => Effect.succeed(false)));
            if (isDirty) {
                return { preservedWorktreePath: worktreePath };
            }
            yield* WorktreeManager.remove(worktreePath).pipe(Effect.catchAll(() => Effect.void));
            return { preservedWorktreePath: undefined };
        }));
    };
