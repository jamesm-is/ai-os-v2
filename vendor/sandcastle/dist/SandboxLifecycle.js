...
const isTransientExecError = (err) => err._tag === "ExecError" &&
...
const execOkWithGitTimeout = (sandbox, command, gitSetupTimeoutMs, options) => execOk(sandbox, command, options).pipe(withTimeout(gitSetupTimeoutMs, () => new GitSetupTimeoutError({
...
export const withSandboxLifecycle = (options, work) => Effect.gen(function* () {
    ...
    yield* display.taskLog("Setting up sandbox", (message) => Effect.gen(function* () {
        ...
        // "dubious ownership".
        yield* execOkWithGitTimeout(sandbox, `git config --global --add safe.directory "${sandboxRepoDir}"`, gitSetupTimeoutMs);
        ...
        if (hostGitName) {
            yield* execOkWithGitTimeout(sandbox, `git config --global user.name "${hostGitName.replace(/"/g, '\\"')}"`, gitSetupTimeoutMs);
        }
        if (hostGitEmail) {
            yield* execOkWithGitTimeout(sandbox, `git config --global user.email "${hostGitEmail.replace(/"/g, '\\"')}"`, gitSetupTimeoutMs);
        }
        // Repo is bind-mounted — discover branch directly
        resolvedBranch = (yield* execOkWithGitTimeout(sandbox, "git rev-parse --abbrev-ref HEAD", gitSetupTimeoutMs, { cwd: sandboxRepoDir })).stdout.trim();
    ...
    if (hostCurrentBranch !== null) {
        ...
        if (hasNewCommits) {
            ...
            }).pipe(withTimeout(mergeToHostTimeoutMs, () => new MergeToHostTimeoutError({
        ...
        }).pipe(withTimeout(commitCollectionTimeoutMs, () => new CommitCollectionTimeoutError({
