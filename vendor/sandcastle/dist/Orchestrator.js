...
import { AgentError, AgentIdleTimeoutError, SessionCaptureError, } from "./errors.js";
...
const invokeAgent = (sandbox, sandboxRepoDir, prompt, provider, idleTimeoutMs, onText, onToolCall, onIdleWarning, idleWarningIntervalMs = IDLE_WARNING_INTERVAL_MS, resumeSession, signal) => Effect.gen(function* () {
    ...
    const resetIdleTimer = () => {
        ...
        timeoutHandle = setTimeout(() => {
            Effect.runPromise(Deferred.fail(timeoutSignal, new AgentIdleTimeoutError({
    ...
    return yield* raced.pipe(Effect.ensuring(Effect.sync(() => {
        abortCleanup?.();
    })));
});
...
export const orchestrate = (options) => {
    ...
    return Effect.gen(function* () {
        ...
        const checkAbort = () => options.signal?.aborted ? Effect.die(options.signal.reason) : Effect.void;
        for (let i = 1; i <= iterations; i++) {
            yield* checkAbort();
            yield* display.status(label(`Iteration ${i}/${iterations}`), "info");
            const sandboxResult = yield* factory.withSandbox(({ hostWorktreePath, sandboxRepoPath, applyToHost, bindMountHandle }) => withSandboxLifecycle({
                hostRepoDir,
                ...
                if (iterationResumeSession &&
                    ...
                    const sbStore = provider.sessionStorage.sandboxStore(ctx.sandboxRepoDir, bindMountHandle);
                    const hStore = provider.sessionStorage.hostStore(hostRepoDir);
                    yield* Effect.tryPromise({
                        try: () => provider.sessionStorage.transfer(hStore, sbStore, iterationResumeSession),
                ...
                const textBuffer = new TextDeltaBuffer((chunk) => {
                    ...
                    Effect.runPromise(streamEmitter.emit({
                        type: "text",
                        message: chunk,
                        iteration: i,
                        timestamp: new Date(),
                ...
                const { result: agentOutput, sessionId, usage: streamUsage, } = yield* invokeAgent(ctx.sandbox, ctx.sandboxRepoDir, fullPrompt, provider, idleTimeoutMs, onText, onToolCall, onIdleWarning, options._idleWarningIntervalMs, iterationResumeSession, options.signal);
                ...
                if (provider.captureSessions &&
                    ...
                    const hStore = provider.sessionStorage.hostStore(hostRepoDir);
                    yield* Effect.tryPromise({
                        try: () => provider.sessionStorage.transfer(sbStore, hStore, sessionId),
                        catch: (e) => new SessionCaptureError({
                    ...
                    if (provider.parseSessionUsage) {
                        const content = yield* Effect.promise(() => hStore
