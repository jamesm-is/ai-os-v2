...
import { formatVolumeMount } from "./mountUtils.js";
...
export const buildImage = (imageName, dockerfileDir, options) => Effect.gen(function* () {
    const buildArgFlags = Object.entries(options?.buildArgs ?? {}).flatMap(([k, v]) => ["--build-arg", `${k}=${v}`]);
...
export const startContainer = (containerName, imageName, env, options) => Effect.gen(function* () {
    ...
    const cpusFlags = options?.cpus !== undefined ? ["--cpus", String(options.cpus)] : [];
    yield* dockerExec([
        "run",
        "-d",
        "--name",
